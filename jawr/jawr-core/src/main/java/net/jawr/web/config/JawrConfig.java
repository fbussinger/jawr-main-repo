/**
 * Copyright 2007-2016 Jordi Hernández Sellés, Ibrahim Chaehoi, Matt Ruby Licensed under the Apache License, Version 2.0
...
public class JawrConfig implements Serializable {
  ...
  /**
   * The property name for the SRI enabling
   */
  public static final String JAWR_SUB_RESOURCE_INTEGRITY_ON = "jawr.sri.on";

  /**
   * The property name for the SRI hash algorithm
   */
  public static final String JAWR_SUB_RESOURCE_INTEGRITY_HASH = "jawr.sri.hash";
  ...
  /**
   * The property name for the jawr working directory. By default it's jawrTemp in the application server working
   * directory associated to the application.
   */
  ...
  /**
   * The property name for the flag indicating if we should use "generator cache".
   */
  ...
  /**
   * The property name for the flag indicating if the CSS image for the CSS retrieved from classpath must be also
  ...
  /**
   * Flag to switch on the strict mode. defaults to false. In strict mode, Jawr checks that the hashcode of the bundle
   * requested is the right one or not.
   */
  ...
  /**
   * Flag which defines if we should process use information of bundle mapping at server startup. This is used to speed
   * up server start up. defaults to true.
   */
  private boolean useBundleMapping = true;

  /**
   * Flag which defines if we should use the smart bundling feature. defaults to true.
   */
  private boolean useSmartBundling = true;

  /**
   * Flag which defines if we should use the generator cache feature. defaults to true.
   */
  ...
  /**
   * Override value to use instead of the context path of the application in generated urls. If null, contextPath is
   * used. If blank, urls are generated to be relative.
   */
  private String contextPathOverride;

  /**
   * Override value to use instead of the context path of the application in generated urls for SSL page. If null,
   * contextPath is used. If blank, urls are generated to be relative.
   */
  ...
  /**
   * The flag indicating that we should use the overridden context path even in debug mode. The default value is false.
   */
  ...
  /**
   * Defines the image hash algorithm. By default the value is CRC32. There are only 2 algorithm available CRC32 and
   * MD5.
   */
  ...
  private final SRIConfig sRIConfig;
  ...
  public JawrConfig(final String resourceType, final Properties props, ConfigPropertyResolver resolver) {
    ...
    this.sRIConfig = new SRIConfig(getBooleanProperty(JAWR_SUB_RESOURCE_INTEGRITY_ON, false), getProperty(JAWR_SUB_RESOURCE_INTEGRITY_HASH));
    ...
    if (StringUtils.isNotEmpty(getProperty("jawr.image.hash.algorithm"))) {
      throw new BundlingProcessException("The property 'jawr.image.hash.algorithm' is not supported anymore, please use '" + JAWR_BINARY_HASH_ALGORITHM + "' instead.");
    }

    if (StringUtils.isNotEmpty(getProperty("jawr.image.resources"))) {
      throw new BundlingProcessException("The property 'jawr.image.resources' is not supported anymore, please use '" + JAWR_BINARY_RESOURCES + "' instead.");
    }
  ...
  /**
   * Get debug mode status. This flag may be overridden using the debugOverrideKey
  ...
  /**
   * Set debug mode.
   * @param debugMode the flag to set
   */
   ...
   * @param useGeneratorCache the flag to set
   */
   ...
   * @param useSmartBundling the flag to set
   */
   ...
   * @param useBundleMapping the flag to set
   */
  ...
  /**
   * Get the the string to use instead of the regular context path. If it is an empty string, urls will be relative to
   * the path (i.e, not start with a slash).
  ...
  /**
   * Set the string to use instead of the regular context path. If it is an empty string, urls will be relative to the
   * path (i.e, not start with a slash).
  ...
  /**
   * Returns true if the URL of the image defines in CSS loaded from classpath, should be overridden for the classpath
   * CSS image servlet.
   * @return true if the image defines in CSS load from classpath, should be overridden for the classpath CSS image
   *         servlet, false otherwise. So if you have a CSS define in a jar file at 'style/default/assets/myStyle.css,
   *         where you have the following statement: background:transparent url(../../img/bkrnd/header_1_sprite.gif)
   *         no-repeat 0 0; Becomes: background:transparent url(getCssImageServletPath()+style/default/
   *         img/bkrnd/header_1_sprite.gif) no-repeat 0 0; And the CSS image servlet will be in charge of loading the
  ...
  /**
   * Set the flag indicating if the URL of the image defines in CSS loaded from classpath, should be overridden for the
   * classpath CSS image servlet.
   * @param classpathCssHandleImage the flag to set So if you have a CSS define in a jar file at
   *          'style/default/assets/myStyle.css, where you have the following statement: background:transparent
   *          url(../../img/bkrnd/header_1_sprite.gif) no-repeat 0 0; Becomes: background:transparent
   *          url(getCssImageServletPath()+style /default/img/bkrnd/header_1_sprite.gif) no-repeat 0 0; And the CSS
   *          image servlet will be in charge of loading the image from the classpath.
   */
   ...
   * @param binaryHashAlgorithm , the hash algorithm to set
   */
   ...
   * @param binaryResourcesDefinition the binary resources definition to set
   */
  ...
  /**
   * Invalidate this configuration. Used to signal objects that have a hold on this instance but cannot be explicitly
  ...
  public final void setCssLinkFlavor(String cssLinkFlavor) {
    if (CSSHTMLBundleLinkRenderer.FLAVORS_HTML.equalsIgnoreCase(cssLinkFlavor) || CSSHTMLBundleLinkRenderer.FLAVORS_XHTML.equalsIgnoreCase(cssLinkFlavor)
  ...
  /**
   * Returns the name of JS engine to use
   * @param defaultJsEnginePropName the default JS engine property name
   * @return the name of JS engine to use
   */
   ...
   * Configuration SRI
   */
  public static class SRIConfig {
    private final boolean enabled;
    private final String hashAlgorithm;

    public SRIConfig(boolean enabled, String hashAlgorithm) {
