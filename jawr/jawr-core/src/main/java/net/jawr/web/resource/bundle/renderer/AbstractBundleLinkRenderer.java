/**
 * Copyright 2007-2016 Jordi Hernández Sellés, Matt Ruby, Ibrahim Chaehoi Licensed under the Apache License, Version 2.0
...
public abstract class AbstractBundleLinkRenderer implements BundleRenderer {
  ...
  protected static final String INTEGRITY_ATTRIBUTE = " integrity=\"$integrity$\"";
  ...
  /*
   * (non-Javadoc)
   * @see net.jawr.web.resource.bundle.renderer.BundleRenderer#renderBundleLinks (java.lang.String,
   * net.jawr.web.resource.bundle.renderer.BundleRendererContext, javax.servlet.jsp.Writer)
   */
  @Override
      ...
      if (bundlePath != null) {
        ...
        if (resourceName != null) {
          ...
          if (bundlePath.isExternalURL()) {
            ...
            out.write(createBundleLink(resourceName, bundlePath.getBundlePrefix(), "d=" + random, contextPath, isSslRequest));
            // } else if (!debugOn && bundlePath.isProductionURL())
            // {
            // out.write(renderLink(resourceName));

          } else if (!debugOn && useGzip) {
            out.write(createGzipBundleLink(resourceName, bundlePath.getBundlePrefix(), contextPath, isSslRequest));
          } else {
            out.write(createBundleLink(resourceName, bundlePath.getBundlePrefix(), null, contextPath, isSslRequest));
          }
  ...
  protected String createGzipBundleLink(String resourceName, String bundlePrefix, String contextPath, boolean isSslRequest) {
    ...
    // dans le cas du gzip comment ca se passe. C'est certainement la servelt qui zip à la volée.
  ...
  protected String createBundleLink(String bundleId, String bundlePrefix, String randomParam, String contextPath, boolean isSslRequest) {
    ...
    if (bundler.getConfig().isDebugModeOn()) {
      ...
      if (StringUtils.isNotEmpty(bundlePrefix)) {
        fullPath = PathNormalizer.joinPaths(bundler.getConfig().getServletMapping(), PathNormalizer.joinPaths(bundlePrefix, path));
      } else {
        fullPath = PathNormalizer.joinPaths(bundler.getConfig().getServletMapping(), path);
      }
    }
    ...
    return renderLink(fullPath, this.bundler.getResourceHash(bundleId));
  }
  ...
  protected abstract String renderLink(String fullPath, final String integrity);

  /**
   * Génération d'un attribut integrity contenant la signature de la ressource à servir
   * @param sb
   * @param integrity
   */
  protected void generateIntegrityAttribute(StringBuilder sb, final String integrity) {
    if (org.apache.commons.lang3.StringUtils.isNoneBlank(integrity)) {
      sb.append(org.apache.commons.lang3.StringUtils.replace(INTEGRITY_ATTRIBUTE, "$integrity$", integrity));
    }
  }
}
